...
/*
 */
public class MsPacManMarco extends Controller<MOVE>
{
...
//				if(move == game.getNextMoveAwayFromTarget(current, game.getCurrentMaze().lairNodeIndex, DM.PATH))
				if(game.getShortestPathDistance(pos, game.getCurrentMaze().lairNodeIndex) > 
				game.getShortestPathDistance(current, game.getCurrentMaze().lairNodeIndex)) // if mspacman goes far from
				{
					cumulativePoints[m] += 200;
					GameView.addPoints(game,Color.green, game.getShortestPath(current, pos));
...
//		}
	...
	private int[] heuristic1(Game game, MOVE[] moves) {
		// if the ghosts are too close and are following Pacman 
		// go the closer safe Power pill and chase them
		int m = 0, pos=0;
		int[] returnValues = new int[2];
		...
		int current = game.getPacmanCurrentNodeIndex();
		boolean isMsPacManInDanger = false;
		
		// heuristic1 comes in the game only if the ghosts are too close
		for(GHOST ghost : GHOST.values()) {
			if(game.getGhostEdibleTime(ghost)==0 && game.getGhostLairTime(ghost)==0) { // check if edible and not in lair
				if(game.getShortestPathDistance(game.getGhostCurrentNodeIndex(ghost), current) < MIN_DISTANCE) { // check if close
						System.out.println("Ghost is close...");
						isMsPacManInDanger = true;
				}
			}
		}
		
		if(isMsPacManInDanger) {
			for(MOVE move: moves) {
				System.out.println(move.toString());
				pos = game.getNeighbour(current, move);			
				
				// A* to get the way out and escape from ghosts -> go to the farthest safe junction
				// from all the ghosts and from there go to the shortest path power pill
				...
				int j=0;
				for(int junct : junctions) {					
					int dist = game.getShortestPathDistance(pos, junct); 
					boolean safe = true;
					
					// Judgement of the goodness of the junction
					for(GHOST ghost : GHOST.values()) { 
						if(game.getGhostEdibleTime(ghost)<30 && game.getGhostLairTime(ghost)==0) {
							if(dist+GUARD_DISTANCE > // if Ms Pacman CANNOT reach the junction before a ghost, safe junction
							game.getShortestPathDistance(game.getGhostCurrentNodeIndex(ghost), junct, game.getGhostLastMoveMade(ghost))) {
								safe = false;
							} else {
								int[] path = game.getShortestPath(pos, junct);
				...
				// Java has problems with arrays
				int[] safeNodes=new int[realSafeJunction.size()];		
				for(int i=0;i<safeNodes.length;i++)
					safeNodes[i]=realSafeJunction.get(i);
				//
				
			
				System.out.println(realSafeJunction.size() + " - " );
				GameView.addPoints(game,Color.cyan, safeNodes);
				
				// ghosts are too close
				// if the next move allow me to escape from non edible and close ghosts gain utility
				// the move must go to the farthest safe junction from  every ghost
				// this is provided by computeWayOut, which computes the junction to escape to and returns
				// the distance and the node to reach
				int[] currentFarthestJunction = computeWayOut(game, safeNodes);
//				System.out.println("junction " + currentFarthestJunction[0] + " is distant from ghosts " + currentFarthestJunction[1]);
				GameView.addPoints(game,Color.red, currentFarthestJunction[0]);
				
				// Consider the closest power pill to the junction, this will be the objective
				...
				boolean areGhostNotEdible = true;
				for(GHOST ghost : GHOST.values()) { 
					if(game.getGhostEdibleTime(ghost)>0 || game.getGhostLairTime(ghost)>0) {
						areGhostNotEdible = false;
					}	
				}
				
				int dist = game.getShortestPathDistance(pos, currentFarthestJunction[0]);
				
//				int closestPowerpill = -1, minDistance = Integer.MAX_VALUE;
//				if(targetsArray.length != 0 && areGhostNotEdible) {
//					// Which is the closest power pill to the junction?
//					for(int powerpill: targetsArray) {
//						if(game.getShortestPathDistance(currentFarthestJunction[0], powerpill) < minDistance) {
//							minDistance = game.getShortestPathDistance(currentFarthestJunction[0], powerpill);
...
//					// visualize in yellow target powerpill
//					GameView.addPoints(game,Color.yellow, closestPowerpill); }
				
				
//				// reward the move that makes PacMan closer to the powerpill
//				if(game.getShortestPathDistance(current, currentFarthestJunction[0]) > dist) {
//					cumulativePoints[m] += 100;
//				}
				
				// The best move is the one that takes to the farthest junction 
				// PROVVISORIO, NON SA COSA FARE DOPO
				// IN PIU' MANCA IL FATTO CHE SPESSO NON TROVA GIUNZIONI SICURE
				// PERCHE' HA SBAGLIATO MOSSA E SI VA A IMBOTTIGLIARE
				// RIVEDERE IL CALCOLO DI WAY OUT
//				 ATTENZIONE NON LO SO and if the junction is reached to the powerpill
				if(game.getNextMoveTowardsTarget(current, currentFarthestJunction[0], DM.PATH) == move) {
					cumulativePoints[m] += 100;
					bestMove = m;
				}
				m++;
		...
		{
			System.out.println("Pacman is safe, heur1 is not adopted");
			// ATTNENZIONE POSSIBILE FONTE DI BUG
			bestMove = 0;
		}
...
//				while(isMsPacManChased(current, game) < 4) // let them enqueue behind you
//					// careful, implement at last
//				{
//					
//				}
				// if they are more than 2 take them to the nearest power pill and eat them
...
//							System.out.println("ghost inbetween");
	...
	// ATTENZIONE ROTTO
	// Se alcuni fantasmi sono lontani e altri sono vicinissimi la distanza media è una giunzione vicina che potrebbe avvicinare pacman al ghost
	// vicino, cioè non sta scappando
	
	private int[] computeWayOut(Game game, int[] safeJunctions) {
		// This function computes the way out when the ghosts are too close and returns the node and the distance to it
		int[] ghostDistancesToJunction = new int[4];
		int maxMeanDistance = 0, currentMeanDistance = 0, maxJunctionNode = 0;
		int[] returnValues = new int[2];
		...
		for(int junct : safeJunctions) {
			...
			for(GHOST ghost : GHOST.values()) { 
				if(game.getGhostEdibleTime(ghost)==0 && game.getGhostLairTime(ghost)==0) {
					ghostDistancesToJunction[j] = game.getShortestPathDistance(game.getGhostCurrentNodeIndex(ghost), junct);
				}
			}
			currentMeanDistance = ghostDistancesToJunction[0] + ghostDistancesToJunction[1] + ghostDistancesToJunction[2] + ghostDistancesToJunction[3];
			currentMeanDistance /= 4;
			if(currentMeanDistance > maxMeanDistance) {
				maxMeanDistance = currentMeanDistance;
				maxJunctionNode = junct;
			}
		}
		returnValues[0] = maxJunctionNode;
		returnValues[1] = maxMeanDistance;
		return returnValues;
	}
